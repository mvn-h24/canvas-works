...
export function CanvasProvider({
  ...
  const [dpr, setDpr] = useState<number>(globalThis?.devicePixelRatio || 1);
  ...
  const notifyView = (w: Matrix) => {
    viewListenersRef.current.forEach((cb) => {
      try {
        cb(w);
      } catch (e) {
        console.error('[onViewChange]', e);
      }
    });
  };
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      setSize((prev) =>
        prev.w !== wCss || prev.h !== hCss
          ? { w: Number(wCss), h: Number(hCss) }
          : prev
      );
    };
    measure();
  ...
  const api = useMemo<CanvasApi>(
    () => ({
      ...
      fitToRect: (r, margin = 24) => {
        ...
        const cx = size.w / 2,
          cy = size.h / 2;
        const wx = (r.x0 + r.x1) / 2,
          wy = (r.y0 + r.y1) / 2;
        const next = mul(
          translate(cx, cy),
          mul(scaleAt(s, s, 0, 0), translate(-wx, -wy))
        );
        setWorldState(() => {
          recomputeMatrices(dpr, next);
          notifyView(next);
